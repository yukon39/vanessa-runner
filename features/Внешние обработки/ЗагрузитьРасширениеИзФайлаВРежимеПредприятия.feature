...
Сценарий: Загрузка расширения из файла с отключением безопасного режима и защиты от опасных действий
    ...
    Дано Я копирую файл "test.cfe" из каталога "tests/fixtures" проекта в рабочий каталог
    ...
    И Я добавляю параметр "Путь=test.cfe;Имя=test;Перезаписывать" для команды "oscript"
    # И Я добавляю параметр "ИмяРасширения=test;БезопасныйРежим;ЗащитаОтОпасныхДействий;ОтключитьЛогикуНачалаРаботыСистемы;ЗавершитьРаботуСистемы" для команды "oscript"
    ...
    Тогда Вывод команды "oscript" содержит
        | ИНФОРМАЦИЯ - Выполняю команду/действие в режиме 1С:Предприятие |
         | Информация: Установлено расширение test, версия |
...
Сценарий: Загрузка расширения из файла - явная установка всех ключей - с отключением безопасного режима и защиты от опасных действий
    ...
    Когда Я добавляю параметр "<КаталогПроекта>/src/main.os run" для команды "oscript"
    И Я добавляю параметр "--command" для команды "oscript"
    И Я добавляю параметр "Путь=test.cfe;Имя=test;Перезаписывать;БезопасныйРежим=Ложь;ЗащитаОтОпасныхДействий=Ложь;ИспользоватьОсновныеРолиДляВсехПользователей=Ложь;ЗавершитьРаботуСистемы" для команды "oscript"
    И Я добавляю параметр "--execute ЗагрузитьРасширениеВРежимеПредприятия.epf" для команды "oscript"
    И Я добавляю параметр " --ibconnection /Fbuild/ib" для команды "oscript"
    И Я добавляю параметр "--language ru" для команды "oscript"
    Когда Я выполняю команду "oscript"
...
Сценарий: Неудачная загрузка несуществующего расширения из файла с отключением безопасного режима и защиты от опасных действий
    ...
    Когда Я добавляю параметр "<КаталогПроекта>/src/main.os run" для команды "oscript"
    И Я добавляю параметр "--command" для команды "oscript"
    И Я добавляю параметр "Путь=Несуществующее.cfe;Имя=Несуществующееtest;Перезаписывать" для команды "oscript"
    И Я добавляю параметр "--execute ЗагрузитьРасширениеВРежимеПредприятия.epf" для команды "oscript"
    И Я добавляю параметр " --ibconnection /Fbuild/ib" для команды "oscript"
    И Я добавляю параметр "--language ru" для команды "oscript"
    Когда Я выполняю команду "oscript"
