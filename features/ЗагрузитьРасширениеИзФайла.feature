...
Контекст:
    ...
    Допустим Я копирую файл "test.cfe" из каталога "tests/fixtures" проекта в рабочий каталог
    И Я очищаю параметры команды "oscript" в контексте

Сценарий: Загрузка одного расширения из файла с обновлением БД
    
    Дано Я выполняю команду "oscript" с параметрами "<КаталогПроекта>/src/main.os loadext --file test.cfe --extension test --updatedb --ibconnection /F./build/ib --language ru"
    И Я показываю вывод команды
    Тогда Код возврата равен 0
    И Я очищаю параметры команды "oscript" в контексте

    Когда Я выполняю команду "oscript" с параметрами "<КаталогПроекта>/src/main.os unloadext ./testNew.cfe test --ibconnection /F./build/ib --language ru"
    И Я показываю вывод команды
...
Сценарий: Загрузка одного расширения из файла без обновления БД
    ...
    Дано Я выполняю команду "oscript" с параметрами "<КаталогПроекта>/src/main.os loadext --file test.cfe --extension test --ibconnection /F./build/ib --language ru"
    И Я показываю вывод команды
    Тогда Код возврата равен 0
    И Я очищаю параметры команды "oscript" в контексте
...
Сценарий: Загрузка одного расширения из файла с обновлением БД без указания имени расширения
    ...
    Дано Я выполняю команду "oscript" с параметрами "<КаталогПроекта>/src/main.os loadext --file test.cfe --updatedb --ibconnection /F./build/ib --language ru"
    И Я показываю вывод команды
